...
$status="0";
$sql = "UPDATE blooddonors SET Status=:status WHERE  id=:eid";
...
$sql = "UPDATE blooddonors SET Status=:status WHERE  id=:aeid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
...
if(isset($_REQUEST['del']))
	{
$did=intval($_GET['del']);
$sql = "delete from blooddonors WHERE  id=:did";
$query = $dbh->prepare($sql);
...
<?php $sql = "SELECT * from  blooddonors ";
$query = $dbh -> prepare($sql);
$query->execute();
